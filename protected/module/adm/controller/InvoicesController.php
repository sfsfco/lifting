 ...
 * @author darkredz
 */
class InvoicesController extends CoreController{
  ...
  if(isset($_GET['load'])){
          ...
          if(isset($_POST['order'])){
            $ordering=array('id','name','serial','delivery_date','client','po','quotation','paid','create_date');
          ...
          $paid=($list->paid=='1')?'checked':'';
          $paidtitle=($list->paid=='1')?'Paid':'Un paid';
            $info['data'][]=array(
              ...
              "<div class='ids text-center'><input type='checkbox' name='active' id='invoice-".$list->id."' title='".$paidtitle."' class='js-switch' ".$paid." /></div>",
              "<div class='ids text-center'>".date('d-m-Y',strtotime($list->create_date))."</div>",
              "<div class='text-center'>
              <a href='".Doo::conf()->APP_URL.Doo::conf()->adminmod."invoices/details/".$list->id."' ><div  class='btn btn-info'><i class='fa fa-print'></i> Details</div></a> 
              </div>"
              );
          }
        }else{
    ...
    public function edit(){
      $id = explode('-',$this->params[0]) ;
      $data['title'] = 'Update Payment';
      $data['id'] = end($id);
      $data['details']=Doo::db()->find('Invoices',array('select'=>'details,paid','where'=>" id='".$data['id']."' ",'limit'=>1));
    ...
    public function update(){
          ...
          $invoices->paid = (isset($_POST['paid']))?'1':'0';
          $invoices->details = $_POST['details'];
          $invoices->edit_date = date('Y-m-d H:m:s');
          $invoices->edit_by = $_SESSION['username']['id'];
          play::pr($_POST);
          echo Doo::db()->update($invoices);
    ...
    public function insert(){
        ...
        $invoices->create_by=$_SESSION['username']['id'];
                     
    $this->db()->insert($invoices);
        
        $_SESSION['inner_message']['success'][]='Data Inserted Successfully';
            header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->adminmod.'invoices' ); 
        }
/*
    public function edit(){
