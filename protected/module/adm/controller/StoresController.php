 ...
 * @author darkredz
 */
class StoresController extends CoreController{
    ...
    if(isset($_GET['load'])){
      ...
      foreach ($lists as $list) {
        ...
        $info['data'][]=array(
          ...
          "<div class='text-center'><a href='".Doo::conf()->APP_URL.Doo::conf()->adminmod."stores/transfare/".$list->id."' ><div  class='btn btn-warning'><i class='fa fa-cog'></i> Transfare</div></a></div>"
          );
      }
    }else{
         $info['data'][]=array("","","","","","","","");
...
public function dotransfare(){
    ...
    if($uu->quantity < $_POST['quantity'] ){
       ...
       $lastinserted=$this->db()->insert($stores);
        ...
        header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->adminmod.'stores' );           
