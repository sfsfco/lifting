...
class UsersController extends CoreController{
    ...
    if(isset($_GET['load'])){
      ...
      foreach ($lists as $list) {
        $info['data'][]=array(
          ...
          "<div class='text-center'><a href='".Doo::conf()->APP_URL.Doo::conf()->adminmod."users/edit/".$list->id."' ><div  class='btn btn-warning'><i class='fa fa-cog'></i> Edit</div></a></div>"
          );
      }
    }else{
         $info['data'][]=array("","","","","","","","");
   ...
   if(isset($this->params[1])){$data['title']='View profies';$data['view']='1';}else{$data['view']='0';}
    $data['user']=Doo::db()->find('Users',array('where'=>" id='".$this->params[0]."'", 'limit'=>1));
      ...
      if(!empty($uu)){
          ...
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->adminmod.'users/add/');
        }else{
        ...
        if(!empty($lastinserted)){
            ...
            header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->adminmod.'users' );           
            }
          }
    ...
    public function update(){
      ...
      if(!empty($uu)){
          $_SESSION['inner_message']['error'][]='Name Already Exist.';
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->adminmod.'users/edit/'.$this->params[0]);
        }else{
