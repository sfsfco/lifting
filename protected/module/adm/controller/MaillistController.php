 ...
 * @author darkredz
 */
class MaillistController extends CoreController{
 ...
 if($_POST['to']=='all'){
     $mails=Doo::db()->find('Maillist',array('select'=>'id,name,email'));
        foreach($mails as $mymail){
            ...
            $mail->addTo($mymail->email,$mymail->name);         
         ...
         $mails=Doo::db()->find('Maillist',array('select'=>'id,name,email','where'=>" id in ('".implode("','", $_POST['to'])."')  "));
        ...
        header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->adminmod.'maillist' );
