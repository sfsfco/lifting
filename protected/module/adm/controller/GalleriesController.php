 ...
 * @author darkredz
 */
class GalleriesController extends CoreController{
    ...
    if(isset($_GET['load'])){
      ...
      foreach ($lists as $list) {
        ...
        $info['data'][]=array(
          ...
          "<div class='text-center'><a href='".Doo::conf()->APP_URL.Doo::conf()->adminmod."galleries/edit/".$list->id."' ><div  class='btn btn-warning'><i class='fa fa-cog'></i> Edit</div></a></div>"
          );
      }
    }else{
         $info['data'][]=array("","","","","","","","");
...
$this->renderc('galleries/index',$data);
 ...
 public function edit(){
    ...
    public function update(){
      ...
      if(!empty($uu)){
            $_SESSION['inner_message']['error'][]='Name Already Exist.';
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->adminmod.'galleries/edit/'.$this->params[0]);
          echo('0');
        }else{
        $galleries = Doo::loadModel('Galleries', true);
      ...
      $galleries->ar_name=$_POST['ar_name'];
        ...
        $galleries->edit_by=$_SESSION['username']['id'];
