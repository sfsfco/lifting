 ...
 * @author darkredz
 */
class ClientsController extends MembersController{
    ...
    if(isset($_GET['load'])){
      ...
      $lists=Doo::db()->find('Clients',array('where'=>$where,'select'=>'id,name,phone,mobile,create_date',key($order)=>$order[key($order)],'limit'=>$limit));
      if(count($lists)>0){
      foreach ($lists as $list) {
        ...
        $info['data'][]=array(
          ...
          "<div class='text-center'><a href='".Doo::conf()->APP_URL.Doo::conf()->membermod."clients/edit/".$list->id."' ><div  class='btn btn-warning'><i class='fa fa-cog'></i> Edit</div></a></div>"
          );
      }
    }else{
         $info['data'][]=array("","","","");
...
$this->renderc('clients/index',$data);
	...
	$data['formUrl']=(Doo::conf()->sef=='0')?Doo::conf()->APP_URL.'index.php/':Doo::conf()->APP_URL;
      ...
      if(!empty($uu)){
          ...
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'clients/add/');
        }else{
  ...
  public function edit(){
    ...
    $data['client']=Doo::db()->find('Clients',array('where'=>"clients.id='".$_SESSION['member_username']['id']."'", 'limit'=>1));
    ...
    if($this->params[0] != $_SESSION['member_username']['id']){
      $_SESSION['inner_message']['error'][]='Invalied Information';
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'clients/edit/'.$_SESSION['member_username']['id']);
    }
      $uu=Doo::db()->find('Clients', array('select'=>'name',
        'where'=>"name='".$_POST['name']."' && id !='".$this->params[0]."'" , 'limit'=>1) );   
      if(!empty($uu)){
          $_SESSION['inner_message']['error'][]='Name Already Exist.';
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'clients/edit/'.$this->params[0]);
        }else{
        $clients = Doo::loadModel('Clients', true);
      ...
      $clients->name=$_POST['name'];
        ...
        $clients->edit_by=$_SESSION['member_username']['id'];
