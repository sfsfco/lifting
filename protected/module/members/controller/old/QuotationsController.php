 ...
 * @author darkredz
 */
class QuotationsController extends MembersController{
    ...
    if(isset($_GET['load'])){
      ...
      foreach ($lists as $list) {
          ...
          if(isset($workorder->id)){
            ...
            $has_workorder="<a href='".Doo::conf()->APP_URL.Doo::conf()->membermod."quotations/convert/".$list->id."' ><div  class='btn btn-danger'><i class='fa fa-share-square'></i> Convert</div></a>";
        }
        ...
        $info['data'][]=array(
          ...
          <a href='".Doo::conf()->APP_URL.Doo::conf()->membermod."quotations/edit/".$list->id."' ><div  class='btn btn-warning'><i class='fa fa-cog'></i> Edit</div></a> 
          <a href='".Doo::conf()->APP_URL.Doo::conf()->membermod."quotations/send/".$list->id."' ><div  class='btn btn-success'><i class='fa fa-send'></i> Send</div></a> 
          <a href='".Doo::conf()->APP_URL.Doo::conf()->membermod."quotations/details/".$list->id."' ><div  class='btn btn-info'><i class='fa fa-print'></i> Details</div></a> 
          ".$has_workorder." 
    ...
    public function insert(){
        ...
        $quotations->create_by=$_SESSION['member_username']['id'];
        $lastinserted=$this->db()->insert($quotations);
          if(!empty($lastinserted)){
            ...
            header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'quotations' );  
        }
...
public function convert(){
 ...
 public function convertnow(){
     ...
     $workorder = Doo::loadModel('Workorders', true);
        ...
        $workorder->create_by=$_SESSION['member_username']['id'];
         ...
         if(!empty($lastinserted)){
             for($x=0;$x<count($data['quotationsdetails']);$x++){
              $workordersdetails = Doo::loadModel('WorkordersDetails', true);
                     ...
                     $workordersdetails->create_by=$_SESSION['member_username']['id'];
