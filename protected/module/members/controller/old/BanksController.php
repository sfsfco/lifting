 ...
 * @author darkredz
 */
class BanksController extends MembersController{
    ...
    if(isset($_GET['load'])){
      ...
      foreach ($lists as $list) {
        $info['data'][]=array(
          ...
          "<div class='text-center'><a href='".Doo::conf()->APP_URL.Doo::conf()->membermod."banks/edit/".$list->id."' ><div  class='btn btn-warning'><i class='fa fa-cog'></i> Edit</div></a></div>"
          );
      }
    }else{
         $info['data'][]=array("","","","");
	...
	$data['formUrl']=(Doo::conf()->sef=='0')?Doo::conf()->APP_URL.'index.php/':Doo::conf()->APP_URL;
      ...
      if(!empty($uu)){
          ...
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'banks/add/');
        }else{
        ...
        if(!empty($lastinserted)){
            ...
            header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'banks' );           
  ...
  public function edit(){
	...
	$data['formUrl']=(Doo::conf()->sef=='0')?Doo::conf()->APP_URL.'index.php/':Doo::conf()->APP_URL;
      ...
      if(!empty($uu)){
            $_SESSION['inner_message']['error'][]='Name Already Exist.';
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'banks/edit/'.$this->params[0]);
          echo('0');
        }else{
        $banks = Doo::loadModel('Banks', true);
