 ...
 * @author darkredz
 */
class StockroomsController extends MembersController{
    ...
    if(isset($_GET['load'])){
      ...
      foreach ($lists as $list) {
        $info['data'][]=array(
          ...
          "<div class='text-center'><a href='".Doo::conf()->APP_URL.Doo::conf()->membermod."stockrooms/edit/".$list->id."' ><div  class='btn btn-warning'><i class='fa fa-cog'></i> Edit</div></a></div>"
          );
      }
    }else{
         $info['data'][]=array("","","","");
  ...
  public function edit(){
    ...
    public function update(){
      ...
      if(!empty($uu)){
           $_SESSION['inner_message']['error'][]='Name Already Exist.';
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'stockrooms/edit/'.$this->params[0]);
        }else{
        $stockrooms = Doo::loadModel('Stockrooms', true);
        $stockrooms->id=$this->params[0];
        $stockrooms->name=$_POST['name'];
        $stockrooms->edit_date=date('Y-m-d H:m:s');
        $stockrooms->edit_by=$_SESSION['member_username']['id'];
