 ...
 * @author darkredz
 */
class ItemsController extends MembersController{
    ...
    if(isset($_GET['load'])){
      ...
      foreach ($lists as $list) {
        $info['data'][]=array(
          ...
          $list->purchase_price,
          $list->order_limit,
          '<div class="text-center"><img src="'.Doo::conf()->APP_URL.Doo::conf()->membermod.'items/printbarcode/'.$list->barcode.'" style="max-width:50px;max-height:50px;"></div>',
          "<div class='text-center'>".date('Y/m/d',strtotime($list->create_date))."</div>",
          "<div class='text-center'><a href='".Doo::conf()->APP_URL.Doo::conf()->membermod."items/edit/".$list->id."' ><div  class='btn btn-warning'><i class='fa fa-cog'></i> Edit</div></a></div>"
          );
      }
    }else{
         $info['data'][]=array("","","","");
	...
	$data['formUrl']=(Doo::conf()->sef=='0')?Doo::conf()->APP_URL.'index.php/':Doo::conf()->APP_URL;
      ...
      if(!empty($uu)){
          ...
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'items/add/');
        }else{
  ...
  public function edit(){
   ...
   $data['item']=Doo::db()->relate('Items','Categories',array('where'=>"items.id='".$this->params[0]."'", 'limit'=>1));
    ...
    public function update(){
      ...
      if(!empty($uu)){
          $_SESSION['inner_message']['error'][]='Name Already Exist.';
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'items/edit/'.$this->params[0]);
        }else{
        $items = Doo::loadModel('Items', true);
        ...
        $items->edit_by=$_SESSION['member_username']['id'];
...
public function printing(){
     $data['rootUrl']=Doo::conf()->APP_URL;
	$data['formUrl']=(Doo::conf()->sef=='0')?Doo::conf()->APP_URL.'index.php/':Doo::conf()->APP_URL;
     echo("<div style='text-align:center'><img src='".$data['formUrl'].Doo::conf()->membermod."items/printbarcode/".$this->params[0]."'><br>".rawurldecode($this->params[1])."</div>");
