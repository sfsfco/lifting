 ...
 * @author darkredz
 */
class CategoriesController extends MembersController{
    ...
    if(isset($_GET['load'])){
      ...
      foreach ($lists as $list) {
        $info['data'][]=array(
          ...
          "<div class='text-center'><a href='".Doo::conf()->APP_URL.Doo::conf()->membermod."categories/edit/".$list->id."' ><div  class='btn btn-warning'><i class='fa fa-cog'></i> Edit</div></a></div>"
          );
      }
    }else{
         $info['data'][]=array("","","","");
...
$this->renderc('categories/index',$data);
  ...
  public function edit(){
	...
	$data['formUrl']=(Doo::conf()->sef=='0')?Doo::conf()->APP_URL.'index.php/':Doo::conf()->APP_URL;
      ...
      if(!empty($uu)){
          $_SESSION['inner_message']['error'][]='Name Already Exist.';
          header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'categories/edit/'.$this->params[0]);
        }else{
        $categories = Doo::loadModel('Categories', true);
      ...
      $categories->name=$_POST['name'];
        ...
        $categories->edit_by=$_SESSION['member_username']['id'];
