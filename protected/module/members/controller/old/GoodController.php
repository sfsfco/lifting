 ...
 * @author darkredz
 */
class GoodController extends MembersController{
    ...
    if(isset($_GET['load'])){
      ...
      foreach ($lists as $list) {
        ...
        $info['data'][]=array(
          ...
          $list->total_value,
          $list->by,
          "<div class='text-center'><a href='".Doo::conf()->APP_URL.Doo::conf()->membermod."good/details/".$list->id."' ><div  class='btn btn-warning'><i class='fa fa-print'></i> Details</div></a></div>"
          );
      }
    }else{
         $info['data'][]=array("","","","","");
...
$this->renderc('good/index',$data);
    ...
    foreach($_POST as $key=>$value){
      ...
      $good = Doo::loadModel('Good', true);
        ...
        $good->create_by=$_SESSION['member_username']['id'];
      ...
      $payments = Doo::loadModel('Payments', true);
        ...
        if(!empty($lastinserted)){
             ...
             for($x=0;$x<$nuu;$x++){
                 ...
                 $gooddetails->create_by=$_SESSION['member_username']['id'];
                ...
                if(!empty($gooddata->id)){
                    ...
                    $goods->edit_by=$_SESSION['member_username']['id'];
                    ...
                    $goods->create_by=$_SESSION['member_username']['id'];
           ...
           $_SESSION['inner_message']['success'][]='Data Added Successfully';
            header( 'Location:'.Doo::conf()->APP_URL.Doo::conf()->membermod.'good' );           
