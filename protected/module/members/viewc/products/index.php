            ...
            var table = $('#datatable-info').DataTable({
                                "initComplete": function(settings, json) {
                                    ...
                                    $('.featured').click(function(event) {
                                        item = $(this);
                                        $.ajax({url: "<?php echo Doo::conf()->APP_URL.Doo::conf()->membermod; ?>products/featured/"+item.attr('icon-val'), success: function(result){
                                            if(result=='0'){
                                                item.html("<i class='fa fa-star-o' aria-hidden='true'></i>");
                                            }else{
                                                item.html("<i class='fa fa-star goldn' aria-hidden='true'></i>");
                                            }
                ...
                "serverSide":true,
                "ajax": {
                    url:"<?php echo Doo::conf()->APP_URL.Doo::conf()->membermod; ?>products/index/?load=1",
                    type:"post",
                    },
                select: true,
                "order": [[ 0, 'desc' ]],
                ...
                buttons: [
                    {
                        ...
                        action: function ( e, dt, node, config ) {
                            window.location.href = "<?php echo Doo::conf()->APP_URL.Doo::conf()->membermod; ?>products/add";
                        },
                        className:'add-blue'
                    },
                    'pageLength',
